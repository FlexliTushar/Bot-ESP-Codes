...
const static uint16_t PT_INTENSITY_WARNING_UPPER_LEVELS[2] = { 700, 800 };    // Upper threshold value for intensity warning levels
const static uint16_t PT_INTENSITY_WARNING_LOWER_LEVELS[2] = { 300, 200 };    // Lower threshold value for intensity warning levels
const static uint16_t FRAME_COUNT_WARNING_LEVELS[2] = { 20, 40 };             // Frame count threshold value for frame count warning levels for 1 m/s.
...
void InitialiseWarningSystemData(const int &index)
{
  if (_photoTransistorCurrentStateArray[index] == 1) {
    if (_photoTransistorCurrentValueArray[index] >= PT_INTENSITY_WARNING_UPPER_LEVELS[1]) {
      _intermColumnPos1Count[index] = 1;
      _intermColumn0Count[index] = 0;
      _intermColumnNeg1Count[index] = 0;
    }
    else if (_photoTransistorCurrentValueArray[index] < PT_INTENSITY_WARNING_UPPER_LEVELS[1] && _photoTransistorCurrentValueArray[index] >= PT_INTENSITY_WARNING_UPPER_LEVELS[0]) {
      _intermColumnPos1Count[index] = 0;
      _intermColumn0Count[index] = 1;
      _intermColumnNeg1Count[index] = 0;
    }
    else if (_photoTransistorCurrentValueArray[index] < PT_INTENSITY_WARNING_UPPER_LEVELS[0]) {
      _intermColumnPos1Count[index] = 0;
      _intermColumn0Count[index] = 0;
      _intermColumnNeg1Count[index] = 1;
    }
  }
  else {
    if (_photoTransistorCurrentValueArray[index] <= PT_INTENSITY_WARNING_LOWER_LEVELS[1]) {
      _intermColumnPos1Count[index] = 1;
      _intermColumn0Count[index] = 0;
      _intermColumnNeg1Count[index] = 0;
    }
    else if (_photoTransistorCurrentValueArray[index] > PT_INTENSITY_WARNING_LOWER_LEVELS[1] && _photoTransistorCurrentValueArray[index] <= PT_INTENSITY_WARNING_LOWER_LEVELS[0]) {
      _intermColumnPos1Count[index] = 0;
      _intermColumn0Count[index] = 1;
      _intermColumnNeg1Count[index] = 0;
    }
    else if (_photoTransistorCurrentValueArray[index] > PT_INTENSITY_WARNING_LOWER_LEVELS[0]) {
      _intermColumnPos1Count[index] = 0;
      _intermColumn0Count[index] = 0;
      _intermColumnNeg1Count[index] = 1;
    }
...
void UpdateWarningSystemData(const int &index) {
  if (_photoTransistorCurrentStateArray[index] == 1) {
    if (_photoTransistorCurrentValueArray[index] >= PT_INTENSITY_WARNING_UPPER_LEVELS[1]) {
      _intermColumnPos1Count[index] += 1;
    }
...
String GetStationCodeFromDataReadByStationSensor() {
  ...
  if (_readingInProcess) {
    ...
    if (_previousIntermediateColumnCode != "000" && _previousIntermediateColumnCode != "111" && _intermediateColumnCode == "111" && _intermediateColumnCode != "000") {
      ...
      for (int i = 0; i < 3; ++i)
      {
        for (int j = 0; j < 3; ++j)
        {
          _warningMatrixData[i][2][j] = _warningMatrixData[i][1][j];
          _warningMatrixData[i][1][j] = _warningMatrixData[i][0][j];
        }
      }
      for (int i = 0; i < 3; ++i)
      {
        _warningMatrixData[i][0][0] = _intermColumnPos1Count[i];
        _warningMatrixData[i][0][1] = _intermColumn0Count[i];
        _warningMatrixData[i][0][2] = _intermColumnNeg1Count[i];
      }
    }
  ...
  if (_rightStationDetected) {
    ...
    String intensityDataLog = "Intensity Warning Data: ";
    int segment = 1;
    for (int i = 2; i >= 0; i--)
    {
      for (int j = 0; j < 3; j++)
      {
        intensityDataLog += "Seg " + String(segment) + ": {";
        for (int k = 0; k < 3; k++)
        {
          intensityDataLog += String(_warningMatrixData[j][i][k]);
          if (k < 2)
          {
            intensityDataLog += ", ";
          }
    ...
    add_log(intensityDataLog);

    InitialiseAllVariables();
    // ComputeHealthOfStationReader();
    // ResetStationReaderVariables();
  }
...
void loop() {
  ...
  if (stationCodeMain != "" && stationCodeMain.length() == 9) { // If station reading is complete only then function will be run, 2 conditions must be satisfied for station reading to be complete, all 3 columns must be read, only then length of stationcode string will be 9, also stationCode must not be empty for station reading to be complete
    add_log("Station code: " + stationCodeMain);
    currentStation = TransformStationCodeIntoStationId(stationCodeMain);
...
void GetDestinationAndJourneyPath() {
  String url = DMS_URL_PREFIX + "GetDestinationStationIdForBot?botId=B1&infeedStationId=I1&strategyType=MyntraSingleScan&retries=0&sectionId=S1";
  int httpCode = 0;
  _http.begin(url);
  add_log("API call!");
  httpCode = _http.GET();
