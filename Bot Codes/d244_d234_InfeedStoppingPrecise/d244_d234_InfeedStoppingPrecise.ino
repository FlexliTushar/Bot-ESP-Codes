...
bool previousCIStateWTM = 0;
bool previousTIStateWTM = 0;
bool onTheColumn = false;

// Handler for the case when TI is OFF
void HandleTrafficIndicatorDetectionOFF(bool columnDetected) {
  if (previousCIStateWTM != columnDetected || onTheColumn) {
...
void HandleTrafficIndicatorDetectionON(bool columnDetected, bool trafficDetected) {
  if (previousCIStateWTM != columnDetected) {
    if (previousTIStateWTM != trafficDetected) {
...
void HandleColumnIndicatorDetectionOFF(bool columnDetected) {
  if (columnDetected == previousCIStateWTM && stopBotFlag && getVelocityActualValueAveraged() == 0) {
    setTagetVelocity(S0_05);
    stopBotFlag = false;
    onTheColumn = false;
  }
...
void loop() {
  ...
  if (columnDetected) {
    ...
    if (trafficDetected) {
      ...
      HandleTrafficIndicatorDetectionON(columnDetected, trafficDetected);
    } else {
      // add_log("Traffic NOT Detected!");
      HandleTrafficIndicatorDetectionOFF(columnDetected);
    }
    previousTIStateWTM = trafficDetected;
    mcp.digitalWrite(PANEL_LED_PIN, HIGH);
  } else {
    HandleColumnIndicatorDetectionOFF(columnDetected);
    previousTIStateWTM = 0;
    mcp.digitalWrite(PANEL_LED_PIN, LOW);
  }
  previousCIStateWTM = columnDetected;
  // -------------------------------------------------------------------------------------------------------------------------------------------
...
void HTTP_DEBUG_LOGGER(void* pvParameters) {
  while (true) {
    ...
    if (debugLoggingString != "B5 " + CODE_ID + ": ") {
