...
void HandleTrafficIndicatorDetectionON() {
  ...
  if (!slowDownFlag) {
    ...
    if (millis() - slowDownCommandStartTime >= ComputeWaitTime() && !stopBotFlag) {
...
void loop() {
  ...
  if (columnDetected) {
      ...
      if (!trafficDetected) {
        if (onTraffic) {
          ...
          if (onTrafficCounter >= 10) {
            onTraffic = false;
            onTrafficCounter = 0;
            // add_log("Previously traffic detected, currently not detected - Same column");
    ...
    if (onColumn) {                   
      ...
      onTraffic = false;
      onTrafficCounter = 0;
      add_log("Column no: " + String(columnNo) + " frame count: " + String(columnCounterForDetection));
