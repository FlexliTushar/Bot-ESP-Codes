...
const unsigned int MIN_THRESHOLD_TIME_FOR_ON_COLUMN_US = 2000;
HTTPClient _http;
...
void HandleTrafficIndicatorDetectionON() {
  ...
  if (!slowDownFlag) {
    // Set the bot to the MINIMAL_SPEED
    add_log("Slowing down to 0.03m/s");
...
bool trafficSeenThisColumn = false;
...
void WTM_OPS(void* pvParameters) {
  while(true) {
    ...
    // --------------------------- Column & Traffic Logic ---------------------------
    if (columnDetectedForCore0) { // Column detected
      // B1
      if (onColumn) { // Already on a column
        // C1
        if (trafficDetectedForCore0 && !trafficSeenThisColumn) { // If traffic detected first time on the given column
          // D1
          onTraffic = true;
          add_log("Traffic is detected");
          taskENTER_CRITICAL(&xWTMMux);
          WTMInAction = true;
          taskEXIT_CRITICAL(&xWTMMux);
          HandleTrafficIndicatorDetectionON();
          trafficSeenThisColumn = true;
        }
        
        if (!trafficDetectedForCore0 && onTraffic && trafficSeenThisColumn) { // If traffic is not detected but detected before on this column - Restarting from the stop state
          // D2
          onTrafficCounter++;
          if (onTrafficCounter >= 10) {
            add_log("Restarting from stop state on column");
            onTraffic = false;
            onTrafficCounter = 0;
            ...
            WTMInAction = false;
            taskEXIT_CRITICAL(&xWTMMux);
            trafficSeenThisColumn = false;
          }
        }
      } 
      else { // New column detected
      // C2
        if (micros() - previousOnColumnTime < MIN_THRESHOLD_TIME_FOR_ON_COLUMN_US) { // Filtering same column to and fro motion
          // taskENTER_CRITICAL(&xErrorMux);
          // errorMode = true;
          // taskEXIT_CRITICAL(&xErrorMux);
          add_log("ER: TOO_FREQUENT_COLUMN_DETECTION");
        } else { // Correct column
          onColumn = true;
          trafficSeenThisColumn = false;
          previousOnColumnTime = micros();
          add_log("CD time @ " + String(micros()));
      ...
      // B2
      if (stopBotFlag && millis() - stopBotFlagStartTime >= 200) { // Check if bot has stopped where no column detection is happening - Correction
        if (getVelocityActualValueAveraged() == 0) {
          // C3
          taskENTER_CRITICAL(&xWTMMux);
          WTMInAction = true;
          taskEXIT_CRITICAL(&xWTMMux);
          add_log("Correction happening");
          setTagetVelocity(S0_03);
          stopBotFlag = false;
        }
      }
      if (onColumn) {
        ...
        // if (!trafficSeenThisColumn && onTraffic) { // If column is crossed whole and traffic is not seen at all on this column
        //   onTraffic = false;
        //   onTrafficCounter = 0;
        //   HandleTrafficIndicatorDetectionOFF();
        //   taskENTER_CRITICAL(&xWTMMux);
