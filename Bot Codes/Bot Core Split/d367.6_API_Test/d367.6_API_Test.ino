...
// Flap function - To turn OFF the electromagnets and open the flap
void OpenFlap() {
  digitalWrite(EMG_PIN, LOW);
}

// Flap function - To command secondary MCU to stay ready for flap open
void StageFlap() {
...
//Flap function - To command secondary MCU to start listening to the control commands
void StartListening() {
  digitalWrite(AFC_PIN_1, HIGH);
  digitalWrite(AFC_PIN_2, HIGH);
}
...
// === Speed Constants ===
// const int SET_SPEED = 3.0f;
const int MIN_SPEED = S0_05;
...
// === Stability Timers ===
unsigned long minSpeedStableSince = 0;
bool minSpeedStable = false;

unsigned long stopSpeedStableSince = 0;
bool stopSpeedStable = false;
...
void WTM_OPS(void* pvParameters) {
  while(true) {
    ...
    // // === Velocity Stability Tracking ===
    // int currentVelocity = getVelocityActualValueAveraged();
    // unsigned long now = millis();
    // if (currentVelocity < MIN_SPEED_THRESHOLD) {
    //   if (minSpeedStableSince == 0) minSpeedStableSince = now;
    //   minSpeedStable = (now - minSpeedStableSince >= STABLE_TIME_MS);
    // } else {
    //   minSpeedStable = false;
    //   minSpeedStableSince = 0;
    // }

    // if (currentVelocity == 0) {
    //   if (stopSpeedStableSince == 0) stopSpeedStableSince = now;
    //   stopSpeedStable = (now - stopSpeedStableSince >= STABLE_TIME_MS);
    // } else {
    //   stopSpeedStable = false;
    //   stopSpeedStableSince = 0;
    // }

    // === Column State ===
    bool newColumnDetected = columnDetectedForCore0 && !lastColumnIndicator;
    bool onColumn = columnDetectedForCore0;
    ...
    //   if (trafficDetectedForCore0) {
    //     if (!isAtMinSpeed) {
    //       setTagetVelocity(MIN_SPEED);
    //       isAtMinSpeed = true;
    //       isStopped = false;
    //     } else if (isAtMinSpeed && minSpeedStable) {
    //       haltMovement();
    //       isStopped = true;
    //     }
    //   } else {
    //     if ((isStopped && stopSpeedStable) || (isAtMinSpeed && minSpeedStable)) {
    //       setTagetVelocity(setSpeed);
    //       isAtMinSpeed = false;
    //       isStopped = false;
    //     }
    ...
    // if (!onColumn && isStopped && stopSpeedStable) {
    //   setTagetVelocity(MIN_SPEED);
    //   isAtMinSpeed = true;
    //   isStopped = false;
    // }

    // // === Exception 2: Resume on Column if Traffic Clears ===
    // if (onColumn && isStopped && stopSpeedStable && !trafficDetectedForCore0) {
    //   setTagetVelocity(setSpeed);
    //   isAtMinSpeed = false;
    //   isStopped = false;
    // }
    ...
    if (newColumnDetected) {
      if (trafficDetectedForCore0) {
        if (!isAtMinSpeed) {
          ...
          isAtMinSpeed = true;
          isStopped = false;
        } else if (isAtMinSpeed && currentVelocity < 30) {
          haltMovement();
          isStopped = true;
        }
      } else {
        // trafficIndicator LOW
        if ((isStopped && currentVelocity == 0) || (isAtMinSpeed && currentVelocity < 30)) {
          setTagetVelocity(setSpeed);
          isAtMinSpeed = false;
          isStopped = false;
        }
    ...
    // ======= Exception 1: Overshoot Recovery =======
    if (!onColumn && isStopped && currentVelocity == 0) {
      setTagetVelocity(MIN_SPEED);
      isAtMinSpeed = true;
      isStopped = false;
    }
    ...
    if (onColumn && isStopped && currentVelocity == 0 && !trafficDetectedForCore0) {
      setTagetVelocity(setSpeed);
      isAtMinSpeed = false;
      isStopped = false;
    }
...
void STATION_DETECTION_OPS(void* pvParameters) {
  while(true) {
    ...
    if (afcPinState && millis() - afcStartTime >= THRESHOLD_TIME_TO_DEFAULT_STATE_OF_AFC) {
      digitalWrite(AFC_PIN_1, LOW);
      digitalWrite(AFC_PIN_2, LOW);
      afcPinState = false;
    }
