...
unordered_map<string, string> STATION_CODE_TO_STATION_ID_MAP = {
  { "100011110", "D1" }, { "101110011", "D2" }, { "101110100", "D3" }, { "101011001", "D4" }, { "101011010", "D5" }, { "100011001", "D6" }, { "110101110", "D7" }, { "100010011", "D8" }, { "100010101", "D9" }, { "100101011", "D10" }, { "100101010", "D11" }, { "100101001", "D12" }, { "100100110", "D13" }, { "100101100", "D14" }, { "100101110", "D15" }, { "100110001", "D16" }, { "100110010", "D17" }, { "100110011", "D18" }, { "100010110", "I1" }, { "100010100", "I11" }, { "110101100", "I12" }
};
...
void HandleTrafficIndicatorDetectionON() {
  ...
  if (!slowDownFlag) {
    ...
    add_log("Slowing doen to 0.05m/s");
  ...
  while(true) {
    ...
    if (columnDetectedForCore0) {
      if (onColumn) {
        if (!trafficDetectedForCore0) {
          ...
          if (onTraffic) {
            if (onTrafficCounter >= 10) {
              onTraffic = false;
              onTrafficCounter = 0;
              add_log("OFF signal while ON - Same Column");
        ...
        if (millis() - previousOnColumnTime < MIN_THRESHOLD_TIME_FOR_ON_COLUMN) {
          taskENTER_CRITICAL(&xErrorMux);
          errorMode = true;
          taskEXIT_CRITICAL(&xErrorMux);
      ...
      if (onColumn) {                   
        ...
        if (stopBotFlag && millis() - stopBotFlagStartTime >= 500) {
