...
bool trackInfeedLatency = false;
unsigned long infeedLatency;
...
void HandleTrafficIndicatorDetectionOFF() {
  ...
  if (slowDownFlag) {
    ...
    if (trackInfeedLatency) {
      add_log("Infeed Latency: " + String(millis() - infeedLatency));
      trackInfeedLatency = 
    }
    ...
    if(!previousCIStateForInfeedAPI) {
      if (startInfeedAPIColumnCounter) {
        ...
        if(infeedAPIcolumnCounter >= 10) {
          ...
          Beep();
          infeedLatency = millis();
          trackInfeedLatency = true;
        }
      }
    }
  }
