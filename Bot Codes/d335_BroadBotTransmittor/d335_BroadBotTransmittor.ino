...
// Constant variables
const String CODE_ID = "d335-WTMv2.2.36v1";
  ...
  for (int i = 0 ; i < columnCount ; i++) {
    ...
    if (botIsPresent[i]) {
      ...
      if (myZoneBotPosition != i) {
        ...
        int skippedColumns = i - myZoneBotPosition - 1;
        if (skippedColumns > 0) {
          // add_log("Number of skipped columns: " + String(skippedColumns));
          // add_log("Detected on column: " + String(i));

          RequestToYourZoneModel errorRequest = IntimateSkippedColumnError(skippedColumns, i);
          AddRequestToYourZoneTable(errorRequest);
          // add_log("Error mode");
          // errorMode = true;
          // errorCode = 1;
        ...
        // add_log("Detected on column: " + String(myZoneBotPosition));
...
void loop() {
  ...
  if(errorMode) {
    ...
    //     zoneMatrix.setPixelColor(i, zoneMatrix.Color(255, virtualZoneMatrix[1][i] * 255, virtualZoneMatrix[2][i] * 255));
    //     zoneMatrix.show();
    //     add_log("Skipped more than " + String(THRESHOLD_TO_SKIP))
    //   }
    // }
  }
  bool botDetectedOnNewColumn = BotDetection();

  myZoneTailLength = TailRegulator(myZoneBotPosition, myZoneBotSpeed, yourZoneBotSpeed);
  pair<int, int> segmentorResult = TailSegmentor(myZoneTailLength, myZoneBotPosition, yourZoneBotMyZoneTailActiveLength_A, yourZoneBotMyZoneTailActiveLength_B);
  myZoneTailActiveLength = segmentorResult.first;
  yourZoneTailLength = segmentorResult.second;
  TailSuperimposer(myZoneBotPosition, myZoneTailActiveLength, myZoneMirrorBotPosition, myZoneMirrorTailActiveLength, yourZoneBotMyZoneTailActiveLength_A, yourZoneBotMyZoneTailActiveLength_B);
