...
#define S0_75 402  // 0.75 m/s
#define S0_8 428   // 0.8 m/s
#define S1 535     // 1 m/s
#define S1_5 802   // 1.5 m/s
#define S2 1070    // 2 m/s
#define S2_2 1177  // 2.2 m/s
#define S2_3 1231  // 2.3 m/s
#define S2_4 1284  // 2.4 m/s
#define S2_5 1337  // 2.5 m/s
#define S2_75 1471 // 2.75 m/s
...
const float R1 = 240000.0;
const float R2 = 10000.0;
const float voltageDividerFactor = (R1 + R2) / R2;
const float L1_BATTERY_THRESHOLD = 51.05;
const float L2_BATTERY_THRESHOLD = 50.05;
const unsigned long THRESHOLD_BUTTON_PRESS_TIME = 2000;
...
unordered_map<string, int> STATION_ID_TO_ZONE_MAP = {
  { "D1", S1 }, { "D2", S2 }, { "D3", S2 }, { "D4", S2 }, { "D5", S2 }, { "D6", S2 }, { "D7", S2 }, { "D8", S2 }, { "D9", S2 }, { "D10", S2 }, { "D11", S2 }, { "D12", S2 }, { "D13", S2 }, { "D14", S2 }, { "D15", S2 }, { "D16", S2 }, { "D17", S2 }, { "D18", S2 }, { "I1", S0_5 }, { "I11", S0_5 }, { "I12", S1 }
};
...
int batteryMonitorCounter = 0;
bool trafficOnColumnCountStart = false;
int trafficOnColumnCounter = 0;
bool allowedToMove = true;
unsigned long allowedToMoveStartTime;
...
void HandleTrafficIndicatorDetectionOFF() {
  ...
  if (slowDownFlag && setSpeed != S0_05 && allowedToMove) {
    // Set the bot to the SET_SPEED
    setTagetVelocity(setSpeed);
    stopBotFlag = false;
    slowDownFlag = false;
    add_log("Starting the bot again @ : " + String(millis()));
  }
}
...
void HandleTrafficIndicatorDetectionON() {
  ...
  if (!slowDownFlag) {
    ...
    setTagetVelocity(S0_05);  // log time here
    slowDownFlag = true;
  } else {
    // Case: If condition true then bot has achieved MINIMAL_SPEED
    if (getVelocityActualValueAveraged() < 30) {     //need to log time here also when speed drops to below 30
      // Case: If condition true then bot has not already stopped
      if (!stopBotFlag) {
        // Stop the bot
        haltMovement(); //time stamp here also
        add_log("Stopping the bot @ : " + String(millis()));
        stopBotFlag = true;
        if(trafficOnColumnCountStart) {
          trafficOnColumnCountStart = false;
          add_log("When given stop command, the column counter: " + String(trafficOnColumnCounter));
          trafficOnColumnCounter = 0;
        }
        if (allowedToMove) {
          allowedToMove = false;
          allowedToMoveStartTime = millis();
        }
      }
    }
  }
...
bool GetCurrentDiverterDirection() {
  int code = 1;
  int count = 0;
  ...
  delay(1);
  if (current_position_front_servo < front_diverter_left_thresold + 30) {
    return 0;
  } else if (current_position_front_servo > front_diverter_right_thresold - 30) {
    return 1;
  }
}

int GetZoneFromStationId(const String &stationCode) {
  // Check if the station code is present in the StationCodeToId map
  string stationCodeString = string(stationCode.c_str());
  if (STATION_ID_TO_ZONE_MAP.find(stationCodeString) != STATION_ID_TO_ZONE_MAP.end()) {
    float direction = STATION_ID_TO_ZONE_MAP.at(stationCodeString);
    return direction;
  }
  return 0;
}
...
void CloseFlap() {
  ...
  //   sm.wheelMode(1);                // Set motor into wheel mode
  //   sm.WriteSpe(1, CLOSE);          // Start the motor with given speed
  //   flapOpen = false;
  //   flapMotorONFlag = true;
  // }
}

void OpenFlap() {
  // if (!flapOpen) {
    digitalWrite(EMG_PIN, LOW);     // Turn OFF the electromagnets
  //   flapOpen = true;
  // }
}

void StageFlap() {
  sm.joinMode(1);                   // Set motor into servo mode
  sm.WritePosEx(1, STAGEPOS, STAGE);// Write the stage value to the servo motor
}

bool IsNonStoppingStation(String stationCode) {
  String stationID = TransformStationCodeIntoStationId(stationCode);
  if (stationID == "D8" || stationID == "D2" || stationID == "I12") {
    return true;
  }
  return false;
}
...
void ErrorHandling() {
  if (errorCode == 3) {
    ...
    unsigned long panelButtonStartTime = millis();
    while (!mcp.digitalRead(PANEL_LED_BUTTON)) {
      // Until panel button is released
    }
    bool direction = GetCurrentDiverterDirection();
    add_log("Current Direction: " + String(direction));
    add_log("Time of button press: " + String(millis() - panelButtonStartTime));
    if (millis() - panelButtonStartTime < THRESHOLD_BUTTON_PRESS_TIME) {
      // Keep the diverter on same side
    } else {
      // Change the diverter direction
      add_log("Changing direction!");
    ...
    String stationCodeMain = "";
    while (stationCodeMain == "" || IsNonStoppingStation(stationCodeMain)) {
    ...
    if (CheckStationValidity(stationCodeMain)) {
      ...
      SetDiverter(direction);
      delay(1000);
      setSpeed = GetZoneFromStationId(currentStation);
      setTagetVelocity(setSpeed);
  ...
  if (errorCode == 6) {
    add_log("ER6: BATTERY_DISCHARGE_ERROR");
    digitalWrite(PANEL_LED_PIN, HIGH);
    while(true) {
      Beep();
      delay(500);
    ...
    if (columnDetected) {
      if(!previousCIStateDiversionCheck) {
          ...
          if(columnCounterDiversioncheck >= 15) {
            ...
            bool direction = GetDirectionFromStationId(currentStation);
            if (direction) {
              if (current_position_front_servo > front_diverter_right_thresold - 10 && current_position_rear_servo < rear_diverter_right_thresold + 10) {
                // Correct diversion
              } else {
                errorMode = true;
                errorCode = 5;
              ...
              if (current_position_front_servo < front_diverter_left_thresold + 10 && current_position_rear_servo > rear_diverter_left_thresold - 10) {
                // Correct diversion
              } else {
                errorMode = true;
                errorCode = 5;
...
// Function to turn OFF the servo motor while flap closing
void StopFlapClosingMotorIfRuning() {
  if (flapMotorONFlag && mcp.digitalRead(LIM_SWITCH)) {
    sm.WriteSpe(1, OFF);
    flapMotorONFlag = false;
  }
}
...
int CheckBatteryLevel(float voltage) {
  if (voltage > L1_BATTERY_THRESHOLD) return 1;
  else if (voltage <= L1_BATTERY_THRESHOLD && voltage > L2_BATTERY_THRESHOLD) return 0;
  else return -1;
}

float GetBatteryVoltage() {
  int16_t adc2;
  float volts2, batteryVoltage;

  adc2 = ads.readADC_SingleEnded(2);
  volts2 = ads.computeVolts(adc2);
  batteryVoltage = volts2 * voltageDividerFactor;
...
int noColumnDetectionIterationNumber = 0;
...
void loop() {
  ...
  int batteryLevel = CheckBatteryLevel(batteryVoltage);
  
  unsigned long loopEndTime= micros();
  unsigned long timeCheck= (loopEndTime - LoopStartTime);
  Serial.println("Battery Function Execution time is :");
  Serial.println(timeCheck);
  if (batteryLevel == 0) {
    if (batteryMonitorCounter%100 == 0) {
      add_log("Battery is in reserve. Voltage (V): " + String(batteryVoltage));
    }
    batteryMonitorCounter++;
  } else if (batteryLevel == -1) {
    add_log("Battery is critically low. Voltage (V): " + String(batteryVoltage));
    errorMode = true;
    errorCode = 6;
  } else {
    batteryMonitorCounter = 0;
  }
    ...
    if (CheckStationValidity(stationCodeMain)) {
      ...
      if (!errorMode) {
        ...
        if (currentStation == "I12") {
          ...
          trafficOnColumnCountStart = true;
          // slowDownFlag = true;
          // setSpeed = S0_05;
        }
  ...
  if (columnDetected) {
    ...
    //       add_log("State of stop bot flag: " + String(stopBotFlag));
    //     }
    //   }

    //   if (trafficOnColumnCountStart) {
    //     trafficOnColumnCounter++;
    //   }
    // }
  } else {
    ...
    if (stopBotFlag && getVelocityActualValueAveraged() == 0) {
      ...
      stopBotFlag = false;
      add_log("Calibrating because bot is between the columns!");
    }
    if (noColumnDetectionIterationNumber%250 == 0) {
      add_log("Not detecting any column!");
    }
    noColumnDetectionIterationNumber++;
  }
  ...
  if (columnDetected) {
    if(!previousCIState) {
      ...
      if (trafficOnColumnCountStart) {
        trafficOnColumnCounter++;
        add_log("count: " + String(trafficOnColumnCounter) + " @ time: " + String(millis()));
      }
    }
  }
  previousCIState = columnDetected;
  ...
  if (!allowedToMove && millis() - allowedToMoveStartTime > 500) {
    allowedToMove = true;
  }
...
void FlapServoConfig() {
  digitalWrite(EMG_PIN, HIGH);
  mcp.pinMode(LIM_SWITCH, INPUT_PULLUP);
  sm.wheelMode(1);
  sm.WriteSpe(1, STAGE);
  while (!mcp.digitalRead(LIM_SWITCH)) {
  }
  mcp.digitalWrite(PANEL_LED_PIN, HIGH);
  sm.WriteSpe(1, OFF);
  sm.joinMode(1);
  sm.CalibrationOfs(1);
  sm.IncreaseLimit(1, MAXLIM);
  delay(1000);
}
