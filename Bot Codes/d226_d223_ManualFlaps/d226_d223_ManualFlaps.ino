    ...
    if (columnDetected) {
      if(!previousCIStateDiversionCheck) {
          ...
          if(columnCounterDiversioncheck >= 15) {
            ...
            bool direction = GetDirectionFromStationId(currentStation);
            if (direction) {
              if (current_position_front_servo > front_diverter_right_thresold - 20 && current_position_rear_servo < rear_diverter_right_thresold + 20) {
                // Correct diversion
              } else {
                errorMode = true;
                errorCode = 5;
              ...
              if (current_position_front_servo < front_diverter_left_thresold + 20 && current_position_rear_servo > rear_diverter_left_thresold - 20) {
                // Correct diversion
              } else {
                errorMode = true;
                errorCode = 5;
  ...
  if (columnDetected) {
    if(!previousCIState) {
      if (startColumnCounter) {
        columnCounter++;
        if(!stopBotFlag && columnCounter >= columnCountThreshold) {
          ReadStationViaPhotoDiode();
          String instColumnCode = String(_photoTransistorCurrentStateArray[0]) + String(_photoTransistorCurrentStateArray[1]) + String(_photoTransistorCurrentStateArray[2]);
          if (instColumnCode == "111") {
            haltMovement();
            ...
            columnCountThreshold = 2;
          } else {
            columnCountThreshold++;
            // if (columnCountThreshold == 5) {
            //   errorMode = true;
            //   errorCode = 6;
            // }
          }
