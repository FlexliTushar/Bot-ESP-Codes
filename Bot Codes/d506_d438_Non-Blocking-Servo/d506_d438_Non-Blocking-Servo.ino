...
// Non-blocking servo communication variables
enum ServoState {
  SERVO_IDLE,
  SERVO_READING_FRONT,
  SERVO_READING_REAR,
...
ServoTask currentServoTask = {SERVO_IDLE, 0, 0, 0, 0, false, false};
const unsigned long SERVO_TIMEOUT_MS = 20; // Reduced timeout per attempt

// Diverter state machine
enum DiverterState {
  DIVERTER_IDLE,
  DIVERTER_READ_FRONT_POS,
  DIVERTER_READ_REAR_POS,
  DIVERTER_MOVE_FRONT,
  DIVERTER_MOVE_REAR,
  DIVERTER_COMPLETE
};

DiverterState currentDiverterState = DIVERTER_IDLE;
bool diverterTargetLeft = false;
...
SMS sm;  // Keep for blocking calls (DiverterTest, etc.)
SMS_NonBlocking smNB;  // Non-blocking servo instance
...
// Non-blocking servo communication functions
void StartServoReadPos(uint8_t servoId) {
  currentServoTask.state = (servoId == FRONT_SERVO) ? SERVO_READING_FRONT : SERVO_READING_REAR;
  currentServoTask.startTime = millis();
  currentServoTask.retryCount = 0;
  currentServoTask.servoId = servoId;
  currentServoTask.isRead = true;
  currentServoTask.success = false;
  
  // Initiate the read using non-blocking library
  int result = smNB.ReadPos_Start(servoId);
  if (result != 1) {
    // Failed to start, mark as failed
    currentServoTask.state = SERVO_FAILED;
  }
}

void StartServoWritePos(uint8_t servoId, int targetPosition, uint16_t speed = 100) {
  currentServoTask.state = (servoId == FRONT_SERVO) ? SERVO_WRITING_FRONT : SERVO_WRITING_REAR;
  currentServoTask.startTime = millis();
  currentServoTask.retryCount = 0;
  currentServoTask.servoId = servoId;
  currentServoTask.targetPosition = targetPosition;
  currentServoTask.isRead = false;
  currentServoTask.success = false;
  
  // Initiate the write using non-blocking library
  int result = smNB.WritePosEx_Start(servoId, targetPosition, speed, 0);
  if (result != 1) {
    currentServoTask.state = SERVO_FAILED;
  }
}

void StartServoEnableTorque(uint8_t servoId, uint8_t enable) {
  currentServoTask.state = (servoId == FRONT_SERVO) ? SERVO_ENABLING_FRONT : SERVO_ENABLING_REAR;
  currentServoTask.startTime = millis();
  currentServoTask.retryCount = 0;
  currentServoTask.servoId = servoId;
  currentServoTask.targetPosition = enable;
  currentServoTask.isRead = false;
  currentServoTask.success = false;
  
  // Initiate the enable/disable using non-blocking library
  int result = smNB.EnableTorque_Start(servoId, enable);
  if (result != 1) {
    currentServoTask.state = SERVO_FAILED;
  }
}

// This function should be called frequently in the loop
bool UpdateServoTask() {
  if (currentServoTask.state == SERVO_IDLE || currentServoTask.state == SERVO_COMPLETE) {
    return true; // Task complete
  }
  
  if (currentServoTask.state == SERVO_FAILED) {
    return false; // Task failed
  }
  
  // Poll the non-blocking library
  int pollResult = smNB.Poll();
  ...
  if (pollResult == 1) {
    // Success!
    if (currentServoTask.state == SERVO_READING_FRONT) {
      current_position_front_servo = smNB.getResult();
    } else if (currentServoTask.state == SERVO_READING_REAR) {
      current_position_rear_servo = smNB.getResult();
    }
    
    currentServoTask.state = SERVO_COMPLETE;
    currentServoTask.success = true;
    smNB.reset();
    return true;
  }
  
  // pollResult == -1: Timeout or error occurred
  currentServoTask.retryCount++;
  
  if (currentServoTask.retryCount >= MAX_REATTEMPTS_FOR_SERVO_COM) {
  ...
  if (currentServoTask.state == SERVO_READING_FRONT || currentServoTask.state == SERVO_READING_REAR) {
    retryResult = smNB.ReadPos_Start(currentServoTask.servoId);
  } else if (currentServoTask.state == SERVO_WRITING_FRONT || currentServoTask.state == SERVO_WRITING_REAR) {
    retryResult = smNB.WritePosEx_Start(currentServoTask.servoId, currentServoTask.targetPosition, 100, 0);
  } else if (currentServoTask.state == SERVO_ENABLING_FRONT || currentServoTask.state == SERVO_ENABLING_REAR) {
    retryResult = smNB.EnableTorque_Start(currentServoTask.servoId, currentServoTask.targetPosition);
  }
  ...
  return false; // Still in progress (retry started)
}

bool IsServoTaskComplete() {
  return currentServoTask.state == SERVO_COMPLETE || currentServoTask.state == SERVO_FAILED;
}

void ResetServoTask() {
  currentServoTask.state = SERVO_IDLE;
  currentServoTask.retryCount = 0;
  currentServoTask.success = false;
}

// Non-blocking diverter functions
void StartDivertLeft() {
  currentDiverterState = DIVERTER_READ_FRONT_POS;
  diverterTargetLeft = true;
  StartServoReadPos(FRONT_SERVO);
}

void StartDivertRight() {
  currentDiverterState = DIVERTER_READ_FRONT_POS;
  diverterTargetLeft = false;
  StartServoReadPos(FRONT_SERVO);
}

// Call this frequently in the loop to update diverter state machine
bool UpdateDiverter() {
  if (currentDiverterState == DIVERTER_IDLE || currentDiverterState == DIVERTER_COMPLETE) {
    return true; // Complete
  }
  
  // Update current servo task
  bool taskComplete = UpdateServoTask();
  
  if (!taskComplete) {
    return false; // Still processing current operation
  }
  
  // Check if task failed
  if (currentServoTask.state == SERVO_FAILED) {
    currentDiverterState = DIVERTER_IDLE;
    return false;
  }
  
  // Task completed successfully - now transition to next state
  // State machine progression (only executed when current task is COMPLETE)
  switch (currentDiverterState) {
    case DIVERTER_READ_FRONT_POS:
      // Front position read complete, now read rear
      ResetServoTask();
      currentDiverterState = DIVERTER_READ_REAR_POS;
      StartServoReadPos(REAR_SERVO);
      return false; // New operation started, not complete yet
      
    case DIVERTER_READ_REAR_POS:
      // Both positions read, now move front servo
      ResetServoTask();
      currentDiverterState = DIVERTER_MOVE_FRONT;
      
      if (diverterTargetLeft) {
        // Moving left
        if (current_position_front_servo < front_diverter_left_thresold) {
          StartServoEnableTorque(FRONT_SERVO, DISABLE);
        } else {
          StartServoWritePos(FRONT_SERVO, front_diverter_left_thresold, 100);
        }
      } else {
        // Moving right
        if (current_position_front_servo > front_diverter_right_thresold) {
          ...
          StartServoWritePos(FRONT_SERVO, front_diverter_right_thresold, 100);
        }
      }
      return false; // New operation started
      
    case DIVERTER_MOVE_FRONT:
      // Front servo moved, now move rear servo
      ResetServoTask();
      currentDiverterState = DIVERTER_MOVE_REAR;
      
      if (diverterTargetLeft) {
        // Moving left
        if (current_position_rear_servo > rear_diverter_left_thresold) {
          StartServoEnableTorque(REAR_SERVO, DISABLE);
        } else {
          StartServoWritePos(REAR_SERVO, rear_diverter_left_thresold, 100);
        }
      } else {
        // Moving right
        if (current_position_rear_servo < rear_diverter_right_thresold) {
          ...
          StartServoWritePos(REAR_SERVO, rear_diverter_right_thresold, 100);
        }
      }
      return false; // New operation started
      
    case DIVERTER_MOVE_REAR:
      // All operations complete
      ResetServoTask();
      currentDiverterState = DIVERTER_COMPLETE;
      return true; // Entire sequence complete
...
bool IsDiverterComplete() {
  return currentDiverterState == DIVERTER_COMPLETE || currentDiverterState == DIVERTER_IDLE;
}
...
// Original blocking Flap and diverter functions (kept for compatibility)
void DivertLeft() {
  int code = 1;
  int count = 0;
...
// Non-blocking version of SetDiverter - call this once to initiate
void SetDiverterNonBlocking(bool direction) {
  if (direction) {
    if (!rightDivertor) {
      StartDivertRight();
      rightDivertor = true;
...
// Original blocking version (kept for compatibility)
...
void loop() {
  // Update non-blocking servo operations
  UpdateDiverter();
  ...
  if (stationCodeMain != "" && stationCodeMain.length() == 9) { // If station reading is complete only then function will be run, 2 conditions must be satisfied for station reading to be complete, all 3 columns must be read, only then length of stationcode string will be 9, also stationCode must not be empty for station reading to be complete
    ...
    if (CheckStationValidity(stationCodeMain)) {
      bool direction = GetDirectionFromStationId(currentStation);
      SetDiverterNonBlocking(direction);  // Use non-blocking version
...
void ServoConfig() {
  ...
  sm.setTimeOut(100);  // Keep for blocking calls
  // smNB.begin(&Serial2, 115200);
  // smNB.setTimeOut(20);  // Short timeout for non-blocking
  // Serial2.begin(115200, SERIAL_8N1, 25, 32);
  // sm.IncreaseLimit(2, 5000);
  // smNB.IncreaseLimit(2, 5000);
}
