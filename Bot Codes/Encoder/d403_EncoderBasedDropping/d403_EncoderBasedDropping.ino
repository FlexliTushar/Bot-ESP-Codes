...
const float DROPPING_CONTROLLER_ERROR_LIMIT = 0.1;
const int PULSE_COUNT_ERROR_LIMIT = 10;
const float H1 = 1.150;
const float H2 = 0.550;
HTTPClient _http;
...
unordered_map<string, float> EDGE_TO_LENGTH_IN_M_MAP = {
  { "I11-D2", 4.845 }, { "D2-D3", 0.985 }, { "D3-D4", 0.948 }, {"D4-D5", 0.984 }, { "D5-D6", 0.980 }, { "D6-D7", 0.945 }, { "D7-D8", 4.146 }, { "D8-D9", 1.063 }, { "D9-D10", 1.064 }, { "D10-D11", 0.903 }, { "D11-D12", 1.022 }, { "D12-D13", 0.857 }, { "D13-D14", 1.064 }, { "D14-D15", 0.980 }, { "D15-D16", 0.986 }, { "D16-D17", 0.903 }, { "D17-D18", 0.984 }, { "D18-D1", 4.193}, { "D1-I1", 5.599}, { "I1-I12", 0.551 }, { "I12-I11", 2.726}
};
...
Adafruit_NeoPixel errorIndicators(INDICATOR_COUNT, INDICATOR_PIN, NEO_GRB + NEO_KHZ800);
...
int lastA = 0;
int lastB = 0;
unsigned long speedUpdaterStartTime = micros();
unsigned long speedUpdaterStopTime = micros();
int previousPulseCount = 0;
float current_encoder_speed = 0;
bool enableDroppingController = false;
int prevStationCount = 0;
int pulseAtReferencePoint = 0;
float timeOfFlight = 0;
float totalDropDistance = 0;
portMUX_TYPE xCurrentStationMux = portMUX_INITIALIZER_UNLOCKED;
String currentStationEncoderTask = "X";
...
//   if (slowDownFlag) {
//     // Set the bot to the SET_SPEED
//     add_log("Setting speed back to set speed!");
//     setTagetVelocity(setSpeed);
//     stopBotFlag = false;
//     if (setSpeed != S0_04) slowDownFlag = false;
//   }
// }
...
//   if (!slowDownFlag) {
//     // Set the bot to the MINIMAL_SPEED
//     add_log("Slowing down to 0.04m/s");
//     setTagetVelocity(S0_04);
//     slowDownFlag = true;
...
bool TrafficIndicatorDetection() {
  ...
  // add_log("Intensity: " + String(TI_rawValue));
...
// Function to set the indicator color
void SetIndicatorColor(INDICATOR_COLORS color) {
  switch (color) {
    case RED:
      for(int i = 0 ; i < LED_INDEX ; i++) {
        errorIndicators.setPixelColor(i, 255, 0, 0);
      }
      errorIndicators.show();
      break;

    case GREEN:
      for(int i = 0 ; i < LED_INDEX ; i++) {
        errorIndicators.setPixelColor(i, 0, 255, 0);
      }
      errorIndicators.show();
      break;
    ...
    case BLUE:
      for(int i = 0 ; i < LED_INDEX ; i++) {
        errorIndicators.setPixelColor(i, 0, 0, 255);
      }
      errorIndicators.show();
      break;
    ...
    case YELLOW:
      for(int i = 0 ; i < LED_INDEX ; i++) {
        errorIndicators.setPixelColor(i, 255, 255, 0);
      }
      errorIndicators.show();
      break;
...
void STATION_DETECTION_OPS(void* pvParameters) {
  while(true) {
    ...
    if (prevDetectedStationCodeCore0 != detectedStationCodeCore0) {
      ...
      // add_log("Edge " + currentStation + "-" + expectedStation);
      // add_log("Pulse Count: " + String(pulseCount - prevStationCount));
      // add_log("Distance (m): " + String((pulseCount - prevStationCount) * 3.14 * 0.1 / 200, 3));
      prevStationCount = pulseCount;
      taskENTER_CRITICAL(&xCurrentStationMux);
      currentStation = TransformStationCodeIntoStationId(detectedStationCodeCore0);
      taskEXIT_CRITICAL(&xCurrentStationMux);
      add_log("Current Station: " + currentStation);
      if (CheckStationValidity(detectedStationCodeCore0)) {
        ...
        if (!errorMode && !WTMInAction) {
          ...
          if (currentStation == "D2") {
            ...
            setSpeedInMPS = 2.5;
            setTagetVelocity(setSpeed);
            enableDroppingController = true;
            pulseAtReferencePoint = GetCurrentPulse();
          }
    ...
    if (previousDroppingStation != dropoffStation) {
      if (dropoffStation == "X") SetIndicatorColor(BLUE);
      else if (dropoffStation == "D18") SetIndicatorColor(YELLOW);
      else SetIndicatorColor(GREEN);
      if (dropoffStation == "D4" || dropoffStation == "D5" || dropoffStation == "D6" || dropoffStation == "D7" || dropoffStation == "D10" || dropoffStation == "D11" || dropoffStation == "D12" || dropoffStation == "D13" || dropoffStation == "D14" || dropoffStation == "D18") {
        timeOfFlight = CalculateTimeOfFlight(H1);
        
      } else if (dropoffStation == "D15" || dropoffStation == "D16" || dropoffStation == "D17") {
        timeOfFlight = CalculateTimeOfFlight(H2);
      }
      totalDropDistance = GetDistanceBetweenStations("I11", dropoffStation);
    ...
    // if (millis() - speedUpdaterStopTime >= 100) {
    //   CurrentSpeedUpdater();
    // }
    ...
    // if (currentStation == "D2" || currentStation == "D8") {
    //   PerformCalibration(GetCurrentPulse() - pulseComputedFromReferencePoint, GetPulseCountBasedOnDistance(GetDistanceBetweenStations("I11", currentStation)));
    // }

    // if (enableDroppingController) {
    //   if (RunDroppingController()) enableDroppingController = false;
    // }

  }
}

TaskHandle_t dropController;
bool droppingEnabled = false;
int pulseComputedFromReferencePoint = 0;

void DROP_CONTROLLER(void* pvParameters) {
  while(true) {
    vTaskDelay(1/portTICK_PERIOD_MS);
    if (micros() - speedUpdaterStartTime >= 100000) {
      CurrentSpeedUpdater();
      // add_log("Actual velocity: " + String(current_encoder_speed, 3) + " Expected velocity: " + String(setSpeedInMPS));
      // add_log("Error: " + String(abs(setSpeedInMPS - current_encoder_speed), 3));
    }

    if(currentStationEncoderTask != "X" && currentStation != currentStationEncoderTask) {
      currentStationEncoderTask = currentStation;
      if (currentStationEncoderTask == "I11") {
        pulseComputedFromReferencePoint = GetCurrentPulse();
        droppingEnabled = true;
      }
      PerformCalibration(GetCurrentPulse() - pulseComputedFromReferencePoint, GetPulseCountBasedOnDistance(GetDistanceBetweenStations("I11", currentStation)));
    }

    if (droppingEnabled && RunDroppingController()) {
...
// Function to configure the color indicators
void NeoPixelConfig() {
  errorIndicators.begin();
  errorIndicators.show();
  errorIndicators.setBrightness(50);
}
...
float GetDistanceBasedOnPulseCount(const int& pulseCount) {
  float distance = (pulseCount * M_PI * 0.1) / 200.0;
  return distance;
}

int GetPulseCountBasedOnDistance(const float& distance) {
  float pulseCount = (distance * 200.0)/(M_PI * 0.1);
  return (int) pulseCount;
}

void CurrentSpeedUpdater() {
  int pulse = GetCurrentPulse();
  speedUpdaterStopTime = micros();
  float distanceTrav = GetDistanceBasedOnPulseCount(pulse - previousPulseCount);
  float time = (float) (speedUpdaterStopTime - speedUpdaterStartTime) / 1000000.0;
  current_encoder_speed = (float) distanceTrav/time;
  speedUpdaterStartTime = speedUpdaterStopTime;
  previousPulseCount = pulse;
}

void PerformCalibration(const int &actualPulseCount, const int &expectedPulseCount) {
  if (actualPulseCount < expectedPulseCount - PULSE_COUNT_ERROR_LIMIT) {
    add_log("Trailing pulse error");
    SetEncoder(GetCurrentPulse() + expectedPulseCount - actualPulseCount);
    previousPulseCount = previousPulseCount + expectedPulseCount - actualPulseCount;
  } else if (actualPulseCount > expectedPulseCount + PULSE_COUNT_ERROR_LIMIT) {
    add_log("Leading pulse error");
    SetEncoder(GetCurrentPulse() - actualPulseCount + expectedPulseCount);
    previousPulseCount = previousPulseCount - expectedPulseCount + actualPulseCount;
  }
}

bool RunDroppingController() {
  float distanceAlreadyTravelledOnEdgeX2 = GetDistanceBasedOnPulseCount(GetCurrentPulse() - pulseAtReferencePoint);
  float advanceDistanceForDroppingX = current_encoder_speed * timeOfFlight;
  float distanceLeftToTravelX1 = totalDropDistance - distanceAlreadyTravelledOnEdgeX2;
  if ((distanceLeftToTravelX1 - advanceDistanceForDroppingX) <= DROPPING_CONTROLLER_ERROR_LIMIT) {
    add_log("Opening flaps...");
    OpenFlap();
    closingFlap = true;
    startClosingTime = millis();
    return true;
...
float CalculateTimeOfFlight(const float &verticalHeight) {
  float timeOfFlight = sqrt((2 * verticalHeight) / 9.8);
  return timeOfFlight;
}

float GetDistanceBetweenStations(String stationA, String stationB) {
  String temp = stationA;
  float d = 0;
  while (temp != stationB) {
    String exp = GetExpectedStationFromStationId(temp);
    String edge = temp + "-" + GetExpectedStationFromStationId(temp);
    string edgeString = string(edge.c_str());
    if (EDGE_TO_LENGTH_IN_M_MAP.find(edgeString) != EDGE_TO_LENGTH_IN_M_MAP.end()) {
      d += EDGE_TO_LENGTH_IN_M_MAP.at(edgeString);
    }
    temp = exp;
  }
  return d;
