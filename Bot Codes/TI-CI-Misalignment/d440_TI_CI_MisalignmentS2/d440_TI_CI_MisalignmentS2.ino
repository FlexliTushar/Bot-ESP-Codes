...
const int STOP_THRESHOLD_INTENSITY = 240;
...
int columnIntensity = 1023;
bool stopWhileStaging = false;
bool stopWhileClosing = false;
bool columnTraversed = false;
bool trafficWasDetected = false;
bool wtmInAction = false;
bool prevColumnDetected = false;
bool columnTraversalInProgress = false;
int trafficFrameCounter = 0;

const int JITTER_LIMIT_PULSES = 5;
const int P_OFFSET_PULSES = 4;
long encoderSnapshotAtStop = 0;
bool awaitingJitterCheck = false;
...
// Validation routine state machine
enum ValidationState {
  VALID_IDLE,
  VALID_START_BACK,
  VALID_WAIT_BACK_CROSS,
  VALID_START_FORWARD,
  VALID_WAIT_FORWARD_CROSS,
  VALID_STOP_AND_ALIGN,
  VALID_FAILED
};
ValidationState validationState = VALID_IDLE;\
...
bool IsBotStopped() {
  if (getVelocityActualValueAveraged() == 0) return true;
  return false;
}

bool ItsAStoppingColumn() {
  if (slowDownFlag && getVelocityActualValueAveraged() < 15 && columnIntensity < STOP_THRESHOLD_INTENSITY) {
    return true;
  }
  return false;
}

// Snapshot encoder at stop decision
void SnapshotEncoderAtStop() {
  encoderSnapshotAtStop = pulseCount;
  awaitingJitterCheck = true;
}

// Return difference (signed) in pulses from snapshot
long EncoderDeltaFromSnapshot() {
  return pulseCount - encoderSnapshotAtStop - P_OFFSET_PULSES;
}

// Jitter correction: move a bit in same direction then reverse at MIN speed
void PerformJitterCorrection() {
  long delta = EncoderDeltaFromSnapshot();
  if (abs(delta) <= JITTER_LIMIT_PULSES) {
    // small jitter - no action
    awaitingJitterCheck = false;
    return;
  }

  // Determine direction of movement: positive -> moved forward; negative -> moved backward
  if (delta > 0) {
    // moved forward: continue reverse @ MIN_SPEED
    if (validationState == VALID_IDLE) validationState = VALID_START_BACK;
  } else {
    // moved backward: continue forward @ MIN_SPEED
    if (validationState == VALID_IDLE) validationState = VALID_START_FORWARD;
  }
  awaitingJitterCheck = false;
}

void HandleWTMCase00(bool columnDetected, bool trafficDetected) {
  if (IsBotStopped() && validationState == VALID_IDLE) {
    add_log("CORRECTION");
    setTagetVelocity(S0_02);
    stopBotFlag = false;
    wtmInAction = true;
  }

  else if (columnTraversed) {
    if (trafficWasDetected) {
      // Case 11 → slowdown
      if (!slowDownFlag) {
        add_log("First time - Traffic detected - Slowing Down!");
        setTagetVelocity(S0_02);
        slowDownFlag = true;
        wtmInAction = true;
      } else {
        // Stop and trigger validation routine
        if (!stopBotFlag) {
          add_log("Second time - Traffic detected - Stopping the Bot!");
          haltMovement();
          stopBotFlag = true;
          wtmInAction = true;
          // start validation state machine
          if (validationState == VALID_IDLE) validationState = VALID_START_BACK;
          if (validationState == VALID_WAIT_FORWARD_CROSS) {
            validationState = VALID_STOP_AND_ALIGN;
            add_log("Validation is complete - Moving back to stop on the column");
            setTagetVelocity(-S0_02);
            stopBotFlag = false;
          }
        }
      ...
      // Case 01 → acceleration
      if (validationState == VALID_WAIT_FORWARD_CROSS) validationState = VALID_IDLE;
      if (validationState == VALID_IDLE) {
        if (stopBotFlag || slowDownFlag) {
          add_log("Traffic OFF - Going to accelerate!");
          setTagetVelocity(setSpeed);
        }
        if (stopBotFlag) stopBotFlag = false;
        if (slowDownFlag && setSpeed != S0_02) slowDownFlag = false;
        wtmInAction = false;
      }
    }
    columnTraversed = false;
  }
}

void HandleWTMCase01(bool columnDetected) {
  // If we detect bot has just been requested to stop previously, snapshot encoder once
  if (IsBotStopped() && wtmInAction && trafficWasDetected && validationState == VALID_IDLE) {
    // Bot stopped on column (previously 11)
    add_log("Traffic OFF - Was already on the column - Going to accelerate!");
    wtmInAction = false;
    trafficWasDetected = false;
    setTagetVelocity(setSpeed);
    if (stopBotFlag) stopBotFlag = false;
    if (slowDownFlag && setSpeed != S0_02) slowDownFlag = false;

    // If we were expecting precision stop earlier, clear awaiting jitter
    awaitingJitterCheck = false;
  } else {
    if (awaitingJitterCheck) {
      PerformJitterCorrection();
    }
  }
}

void HandleWTMCase10(bool columnDetected) {
  if (awaitingJitterCheck) {
    PerformJitterCorrection();
  }
}

void HandleWTMCase11() {
  if (ItsAStoppingColumn() && !stopBotFlag && validationState == VALID_STOP_AND_ALIGN) {
    validationState = VALID_IDLE;
    add_log("Stopping the bot @ column");
    haltMovement();
    stopBotFlag = true;
    wtmInAction = true;
    // snapshot encoder to detect jitter and for validation
    SnapshotEncoderAtStop();
  }
}

void UpdateColumnTraversalState(bool columnDetected, bool trafficDetected) {
  // Start of a new column
  if (columnDetected && !prevColumnDetected && !columnTraversalInProgress) {
    columnTraversalInProgress = true;
    trafficFrameCounter = 0;
    trafficWasDetected = false;
  }

  // During traversal
  if (columnTraversalInProgress) {
    if (trafficDetected) {
      trafficFrameCounter++;
      if (trafficFrameCounter > 1 && !trafficWasDetected) {
        trafficWasDetected = true;
      }
    }
  }
  ...
  // End of column
  if (!columnDetected && prevColumnDetected && columnTraversalInProgress) {
    columnTraversed = true;
    columnTraversalInProgress = false;
  }

  prevColumnDetected = columnDetected;
}

// Call this at the end of handle_TI_CI_logic() each cycle
void RunValidationRoutine(bool columnDetected, bool trafficDetected) {
  ...
  switch (validationState) {
    ...
    case VALID_START_BACK:
      // Start moving backward slowly to cross the column
      add_log("Validation: start back");
      setTagetVelocity(-S0_02);
      validationState = VALID_WAIT_BACK_CROSS;
      stopBotFlag = false;
      break;

    case VALID_WAIT_BACK_CROSS:
      // Wait until we see CI pattern (column crossed backwards)
      if (!columnDetected && prevColumnDetected) {
        // crossed backwards (CI 1->0) — treat as crossed
        add_log("Validation: crossed back");
        validationState = VALID_START_FORWARD;
      }
      break;

    case VALID_START_FORWARD:
      // Move forward again to re-enter column
      add_log("Validation: start forward");
      setTagetVelocity(S0_02);
      validationState = VALID_WAIT_FORWARD_CROSS;
      break;
  }
}
...
void loop() {
  ...
  if (stationCodeMain != "" && stationCodeMain.length() == 9) { // If station reading is complete only then function will be run, 2 conditions must be satisfied for station reading to be complete, all 3 columns must be read, only then length of stationcode string will be 9, also stationCode must not be empty for station reading to be complete
    ...
    if (CheckStationValidity(stationCodeMain)) {
      ...
      if (!errorMode) {
        if (currentStation == "D1" && !wtmInAction) {
          setSpeed = S1;
          setTagetVelocity(setSpeed);
        }

        if (currentStation == "D2" && !wtmInAction) {
          setSpeed = S2;
          setTagetVelocity(setSpeed);
        }

        if (currentStation == "I1" && !wtmInAction) {
          setSpeed = S0_3;
          setTagetVelocity(setSpeed);
          packetCount++;
        }

        if (currentStation == "I12" && !wtmInAction) {
          setSpeed = S0_02;
          setTagetVelocity(setSpeed);
          infeedDetected = true;
        ...
        if (currentStation == "I11" && !wtmInAction) {
          setSpeed = S1;
          setTagetVelocity(setSpeed);
          StageFlap();
  ...
  UpdateColumnTraversalState(columnDetected, trafficDetected);

  // --- Main case handling ---
  if (!trafficDetected && !columnDetected) {            // Case 00
    HandleWTMCase00(columnDetected, trafficDetected);
  } 
  else if (trafficDetected && !columnDetected) {        // Case 10
    HandleWTMCase10(columnDetected);
  } 
  else if (!trafficDetected && columnDetected) {        // Case 01
    HandleWTMCase01(columnDetected);
  } 
  else if (trafficDetected && columnDetected) {         // Case 11
    HandleWTMCase11();
  }

  RunValidationRoutine(columnDetected, trafficDetected);
  // -------------------------------------------------------------------------------------------------------------------------------------------
