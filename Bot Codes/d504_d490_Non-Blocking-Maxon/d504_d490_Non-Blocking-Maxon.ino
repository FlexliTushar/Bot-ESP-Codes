...
unordered_map<string, int> STATION_ID_TO_SPEED_MAP = {
  { "S1", S0_5 }, { "D1", S0_5 }, { "D2", S0_5 }, { "D3", S2 }, { "D4", S2 }, { "D5", S2 }, { "D6", S2 }, { "D7", S2 }, { "D8", S2 }, { "D9", S2 }, { "D10", S2 }, { "D11", S2 }, { "D12", S2 }, { "D13", S2 }, { "D14", S2 }, { "D15", S2 }, { "D16", S2 }, { "D17", S2 }, { "D18", S2 }, { "I1", S0_3 }, { "I11", S1 }, { "I12", S1 }
};
...
void loop() {
  // Update non-blocking velocity command state machine
  updateVelocityCommand();
  
  // Update non-blocking velocity read state machine
  updateReadVelocityActualValueAveraged();
  
  // Check if velocity read completed and log the result (check every loop iteration)
  if (isVelocityReadComplete() || isVelocityReadTimeout()) {
    add_log(getVelocityReadResult());
  }
  ...
  if (stationCodeMain != "" && stationCodeMain.length() == 9) {
    ...
    // Start non-blocking velocity read when station is detected
    if (!isVelocityReadBusy()) {
      startReadVelocityActualValueAveraged();
    }
