...
int GetExpectedRPM(const int &time)
{
  float timeInSec = time/1000.0;
  float timeToAcc = 2/1.87;
  if (timeInSec <= timeToAcc) {
    float speed = 1.87 * timeInSec;
    int rpm = (speed * 2.8 * 60) / (M_PI * 0.1);
    return rpm;
  } else if (timeInSec > timeToAcc && timeInSec <= 5) {
    return S2;
  } else if (timeInSec > 5) {
    float speed = 2 - 1.87 * (timeInSec - 5);
    if (speed < 0) speed = 0;
    int rpm = (speed * 2.8 * 60) / (M_PI * 0.1);
    return rpm;
  }
...
unsigned long GetExpectedPulses(const int &time)
{
  float timeInSec = time/1000.0;
  float timeToAcc = 2/1.87;
  if (timeInSec <= timeToAcc) {
    float distance = 0.5 * 1.87 * timeInSec * timeInSec;
    unsigned long pulseCount = (distance * 2.8 * 70400) / (M_PI * 0.1);
    return pulseCount;
  } else if (timeInSec > timeToAcc && timeInSec <= 5 - timeToAcc) {
    float distance = 0.5 * 1.87 * timeToAcc * timeToAcc + 2 * (timeInSec - timeToAcc);
    unsigned long pulseCount = (distance * 2.8 * 70400) / (M_PI * 0.1);
    return pulseCount;
  } else if (timeInSec > 5 && timeInSec <= 5 + timeToAcc) {
    float distance = 0.5 * 1.87 * timeToAcc * timeToAcc + 2 * (5 - timeToAcc) + 2 * (timeInSec - 5) - 0.5 * 1.87 * (timeInSec - 5) * (timeInSec - 5);
    unsigned long pulseCount = (distance * 2.8 * 70400) / (M_PI * 0.1);
    return pulseCount;
  } else if (timeInSec > 5 + timeToAcc) {
    float distance = 0.5 * 1.87 * timeToAcc * timeToAcc + 2 * (5 - timeToAcc) + 2 * timeToAcc - 0.5 * 1.87 * timeToAcc * timeToAcc;
    unsigned long pulseCount = (distance * 2.8 * 70400) / (M_PI * 0.1);
    return pulseCount;
  }
}
...
void loop() {
  // Serial.println(getPositionActualValue());
  unsigned long now = millis();
  if (now - startTime <= 5000) {
    // int32_t actualRPM = getVelocityActualValueAveraged();
  } else {
    haltMovement();
    // while (1); // End test
  }
  if (now - lastLogTime >= 100) {
    int32_t actualRPM = getVelocityActualValueAveraged();
    unsigned long actualPulseCount = getPositionActualValue();
    // add_log(String(now - startTime) + "," + String(GetExpectedRPM(now - startTime)) + "," + String(actualRPM) + "," + String(GetExpectedPulses(now - startTime)) + "," + String(actualPulseCount));
    add_log(String(now - startTime) + "," + String(GetExpectedRPM(now - startTime)) + "," + String(actualRPM));
    lastLogTime = now;
  }
}
